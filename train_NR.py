...
import pickle
import numpy as np
from data_boot_gs import FaceImageIter
...
class LossValue1(mx.metric.EvalMetric):
    def __init__(self):
        self.axis = 1
        super(LossValue1, self).__init__(
            name='softmaxloss1', axis=self.axis,
            output_names=None, label_names=None)
        self.eps = 0
    def update(self, labels, preds):
        #print("preds: ", preds[1],preds[2],preds[3])
        preds = [preds[1]]  # use softmax output
...
class LossValue2(mx.metric.EvalMetric):
    def __init__(self):
        ...
        super(LossValue2, self).__init__(
            name='softmaxloss2', axis=self.axis,
            output_names=None, label_names=None)
    def update(self, labels, preds):
...
def parse_args():
  ...
  parser.add_argument('--batch_size_mining', type=int, default=20, help='embedding length')
  ...
  parser.add_argument('--noise-beta', type=float, default=1, help='param for boot')
  ...
  parser.add_argument('--bin-dir', type=str, default='/home/zhongyaoyao/insightface/src', help='directory of the log file')
...
def get_symbol(args, arg_params, aux_params):
  ...
  if args.loss_type==0: #softmax
    ...
    if args.margin_a!=1.0 or args.margin_m!=0.0 or args.margin_b!=0.0:
      if args.margin_a==1.0 and args.margin_m==0.0:
        ...
        body = mx.sym.broadcast_mul(gt_one_hot, diff)
        fc7 = fc7+body
  out_list = [mx.symbol.BlockGrad(embedding)]#0
  ...
  out_list.append(softmaxloss)#3

  '''
  softP = mx.sym.softmax(data=fc7)
  out_list.append(mx.symbol.BlockGrad(softP))  # 2
  a = mx.sym.Variable("a", shape=(1,), init=mx.init.Uniform(10))
  #softP = mx.symbol.L2Normalization(softP, mode='instance', name='fcn')
  gt_one_hot = mx.sym.one_hot(gt_label, depth=args.num_classes, on_value=1.0, off_value=0.0)
  #cross_entropy = mx.sym.broadcast_mul(gt_one_hot, mx.sym.log(mx.symbol.BlockGrad(softP)))
  cross_entropy = -mx.sym.broadcast_mul(gt_one_hot, mx.sym.log(mx.sym.broadcast_mul(a, mx.symbol.BlockGrad(softP))))
  softmaxloss = mx.sym.MakeLoss(cross_entropy)
...
def train_net(args):
    ...
    '''
    train_dataiter = FaceImageIter(
        mx_pretrained   = args.pretrained,
        ctx                  = ctx,
        ctx_num              = args.ctx_num,
        batch_size          = args.batch_size,
        batch_size_mining    = args.batch_size_mining,
        data_shape           = data_shape,
        path_imgrec          = path_imgrec,
        shuffle              = True,
        rand_mirror          = args.rand_mirror,
    ...
    eval_metrics = [mx.metric.create(AccMetric()),mx.metric.create(LossValue2())]
    ...
    for name in args.target.split(','):
      ...
      #path = os.path.join("/ssd/MF2_aligned_pic10/",name+".bin")
